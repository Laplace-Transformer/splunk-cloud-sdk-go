...
func Test_global_flag_invalid_host_url(t *testing.T) {
	...
	searchString := "HTTPS://INVALIDHOSTURL:443"
...
func Test_global_flag_auth_url(t *testing.T) {
	...
	command := "login --use-pkce --pwd " + password + " --auth-url " + utils.IdpHost
	searchString := ""
	success := utils.Execute_cmd_with_global_flags(command, searchString, t, false)
	assert.Equal(t, true, success)
...
func Test_global_flag_invalid_auth_url(t *testing.T) {
	password := "\"" + utils.Password + "\""
	command := "login --use-pkce --pwd " + password + " --auth-url " + utils.InvalidAuthUrl
...
func Test_global_flag_tenant(t *testing.T) {
	command := "identity get-role --role identity.admin --tenant " + utils.TestTenant
...
func TestGetSystemTenantWithGlobalEnv(t *testing.T) {
	t.Skip("Remove skip when timeout to in TestGetSystemTenantWithGlobalEnv is investigated")
...
func TestGetSystemTenantWithoutGlobalEnv(t *testing.T) {
	t.Skip("Remove skip when timeout to in TestGetSystemTenantWithoutGlobalEnv is investigated")
...
func TestGlobalRegionFlag(t *testing.T) {
	t.Skip("Remove skip when timeout to in TestGlobalRegionFlag is investigated")
